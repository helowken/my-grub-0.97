...
static int real_get_cmdline(char *prompt, char *cmdline, int maxlen,
	 ...
	 *
	 * Each section is basically 77 or less characters, but the exeption
	 * is the first section, which is 78 or less characters, because the
	 * starting point is speial. See below.
	 *
	 ...
	 * is put at the margin which is shared by the first section and the
...
#define CMDLINE_MARGIN	10
	...
	void cl_refresh(int full, int len) {
		...
		/* Compute the index of start writing BUF and the resulting position
		 * on the screen. */
		if (section == 0) {
			int offset = 0;
	...
	while ((c = ASCII_CHAR(getkey())) != '\n' && c != '\r') {
		/* READLINE is non-zero, handle readline-like key bindings. */
		if (readline) {
			switch (c) {
...
# elif defined(HAVE_USCOPE_START_SYMBOL)
