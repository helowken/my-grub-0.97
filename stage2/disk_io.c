...
int next_partition(unsigned long drive, unsigned long dest,
	...
	int next_pc_slice(void) {
		...
		*len = PC_SLICE_LENGTH(buf, *entry);
		 ...
		 * the rather odd definition of extended partitions. Enve worse,
...
int set_bootdev(int hdbias) {
	...
	if ((saved_drive & 0x80) && cur_part_addr) {
		if (rawread(saved_drive, cur_part_offset, 0, 
						SECTOR_SIZE, (char *) SCRATCHADDR)) {
			char *dst, *src;

			/* Need only the partition table.
			 * XXX: We cannot use grub_memmove because BOOT_PRT_TABLE
