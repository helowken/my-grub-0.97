...
char *err_list[] = 
{
	...
	[ERR_NO_DISK_SPACE] = "No space sectors on the disk",
	[ERR_NO_PART] = "No such partition",
	[ERR_NUMBER_OVERFLOW] = "Overflow while parsing number",
	[ERR_NUMBER_PARSING] = "Error while parsing number",
	[ERR_OUTSIDE_PART] = "Attempt to acess block outside partition",
 ...
 * Thus this kludge. */
static unsigned long mmap_avail_at(unsigned long bottom) {
...
void init_bios_info() {
	...
	do {
		...
		mbi.mmap_length += *((unsigned long *) addr) + 4;	/* 4 is the DESC_LEN */
		addr += *((unsigned long *) addr) + 4;
	} while (cont);
	...
	if (mbi.mmap_length) {
		...
		/* See BIOS Func: INT 0x15, AX = 0xE801 */
		cont = memtmp & ~0xFFFF;
		memtmp = memtmp & 0xFFFF;

		if (cont != 0)
		  /* CONT has been converted to KB in get_eisamemsize() */
