...
#define MENU_WIDTH	(MENU_RIGHT - MENU_LEFT - 1)
#define MENU_ENTRY_X	(MENU_LEFT + 2)
#define MENU_ENTRY_RX	(MENU_RIGHT - 3)
#define MENU_TOP	3
#define MENU_BOTTOM	(MENU_TOP + 2 + MENU_SIZE)
#define MENU_ENTRY_Y	(MENU_TOP + 1)
#define MENU_COMMENT_HEIGHT	3
...
static void print_entry(int y, int highlight, char *entry) {
	...
	grub_putchar(' ');
	for (x = MENU_ENTRY_X; x < MENU_RIGHT; x++) {
		if (*entry && x <= MENU_ENTRY_RX) {
			if (x == MENU_ENTRY_RX)
...
static void print_entries(int y, int size, int first, int entryno, 
						char *menu_entries) {
	int i, rx;
	...
	gotoxy(MENU_RIGHT - 1, y + entryno + 1);
}
...
static void print_border(int y, int size) {
	...
	i = 1;
	while (1) {
		gotoxy(MENU_LEFT, y + i);

		if (i > size)
		  break;

		grub_putchar(DISP_VERT);
		gotoxy(MENU_RIGHT, y + i);
		grub_putchar(DISP_VERT);
...
static void run_menu(char *menu_entries, char *config_entries, int num_entries,
					char *heap, int entryno) {
	int c, time1, time2 = -1;
	int first_entry = 0;	/* which entry started to print */
	char *cur_entry = 0;
...
restart:
	...
	if (! show_menu) {
		...
		while (1) {
			...
			if (checkkey() != -1 && ASCII_CHAR(getkey()) == 27) {	
				grub_timeout = -1;
				show_menu = 1;
				break;
			}
	...
	if (show_menu) {
      ...
      boot sequence, \'c\' for a command-line, \'o\' to open a newline\n\
      ...
      selected line, or escape to go back to the main menu.");
		...
		if (current_term->flags & TERM_DUMB)
		  ...
		  print_entries(MENU_TOP, MENU_SIZE, first_entry, entryno, menu_entries);
	}
	...
	while (1) {
		...
		if (checkkey() >= 0 || grub_timeout < 0) {
			...
			if (c == 16 || c == '^') {	/* 16: KEY_UP, see asm.S */
				if (current_term->flags & TERM_DUMB) {
					if (entryno > 0)
					  entryno--;
				} else {
					if (entryno > 0) {
						/* cancel highlight the old one */
						print_entry(MENU_ENTRY_Y + entryno, 0,
								get_entry(menu_entries, first_entry + entryno, 0));
						entryno--;
						/* highlight the new one */
						print_entry(MENU_ENTRY_Y + entryno, 1,
								get_entry(menu_entries, first_entry + entryno, 0));
					} else if (first_entry > 0) {
						first_entry--;
						print_entries(MENU_TOP, MENU_SIZE, first_entry, entryno,
									menu_entries);
					}
				}
			} else if ((c == 14 || c == 'v') &&		/* 14: KEY_DOWN, see asm.S */
						first_entry + entryno + 1 < num_entries) {
				if (current_term->flags & TERM_DUMB)
				  entryno++;
				else {
					if (entryno < MENU_SIZE - 1) {
						/* cancel highlight the old one */
						print_entry(MENU_ENTRY_Y + entryno, 0,
								get_entry(menu_entries, first_entry + entryno, 0));
						entryno++;
						...
						print_entry(MENU_ENTRY_Y + entryno, 1,
								get_entry(menu_entries, first_entry + entryno, 0));
					} else if (num_entries > MENU_SIZE + first_entry) {
						first_entry++;
						print_entries(MENU_TOP, MENU_SIZE, first_entry, entryno,
									menu_entries);
					}
				}
			} else if (c == 7) {	/* 7: PageUp, see asm.S */
				/* Page Up */
				first_entry -= MENU_SIZE;
				if (first_entry < 0) {
					entryno += first_entry;
					first_entry = 0;
					if (entryno < 0)
				...
				print_entries(MENU_TOP, MENU_SIZE, first_entry, entryno, menu_entries);
			} else if (c == 3) {	/* 3: PageDown, see asm.S */
				/* Page Down */
				first_entry += MENU_SIZE;
				if (first_entry + entryno + 1 >= num_entries) {
					first_entry = num_entries - MENU_SIZE;
					if (first_entry < 0)
					  first_entry = 0;
			...
			if (config_entries) {	/* in menu list */
				if (c == '\n' || c == '\r' || c == 6)	/* 6: KEY_RIGHT */
				  break;
			} else {	/* in config entry menu list */
				if (c == 'd' || c == 'o' || c == 'O') {
					if (! (current_term->flags & TERM_DUMB)) 
					  print_entry(MENU_ENTRY_Y + entryno, 0,
								get_entry(menu_entries, first_entry + entryno, 0));
					...
					if (c == 'o') {
						...
						if (entryno < MENU_SIZE - 1 || (current_term->flags & TERM_DUMB))
						  entryno++;
						else 
						  first_entry++;
					...
					if (c == 'O') {	/* insert */
						...
						if (entryno < 0)
						  entryno = 0;
						if (first_entry && first_entry + MENU_SIZE > num_entries)
						  first_entry--;
					}
			...
			if (! auth && password) {
				...
				if (c == 'e') {
					...
					if (config_entries) {	/* get config entries of current menu entry */
						new_heap = heap;
					...
					/* copy config entries or the current edit entry to NEW_HEAP. */
					do {
					...
					*(new_heap++) = 0;
					...
					if (config_entries)	/* show the config entry menu list */
			...
			*default_file = 0;
			...
			do {
				...
				if (state > 1) {
					...
					/* Title without configs is not allowed. */
					menu_len = prev_menu_len;
					config_len = prev_config_len;
				}
					...
					for (i = 0; i < MAX_FALLBACK_ENTRIES; i++) {
						...
						if (fallback_entries[i] >= num_entries) {
							grub_memmove(fallback_entries + i,
										...
										((MAX_FALLBACK_ENTRIES - i - 1) * sizeof(int)));
							i--;
						}
					}
